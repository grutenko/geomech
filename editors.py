...
__xConfirm__ = wx.NewEventType()

class xConfirm(wx.PyEvent):
    def __init__(self, **kw):
        wx.PyEvent.__init__(self)
        self.SetEventType(__xConfirm__)
        self._getAttrDict().update(kw)

EVT_EDITOR_CONFIRM = wx.PyEventBinder(__xConfirm__)

class Base:
    _entity: database.Base = None
    _on_save: Callable[[database.Base], None] = None

    def set_on_save(self, on_save: Callable[[database.Base], None]): 
        self._on_save = on_save

    def _configure_(self):
        raise NotImplementedError("Not implemented _configure_() in {}, {}.".format(__file__, __class__.__name__))
    
    def _write_entity_(self, e: database.Base) -> database.Base:
        raise NotImplementedError("Not implemented _write_entity_() in {}, {}.".format(__file__, __class__.__name__))
    
    def _set_fields_(self, e: database.Base):
        raise NotImplementedError("Not implemented _set_fields_() in {}, {}.".format(__file__, __class__.__name__))
...
class DischargeMeasurement_Editor(Base, Ui_DischargeMeasurement_Editor, mixins.OptionalFieldsMixin):
    def _configure_(self):
    ...
    def _set_fields_(self, e: database.Base):
        ...
        _set('field_RID', e.RID)
        _set('field_DSID', e.discharge_series)
        self.field_DSID.Enable(False)
        _set('field_SNumber', e.SNumber)
        _set('field_DschNumber', e.DschNumber)
        _set('field_CoreNumber', e.CoreNumber)
        _set('field_Diameter', e.Diameter)
        _set('field_Length', e.Length)
        _set('field_Weight', e.Weight)
        _set('field_CoreDepth', e.CoreDepth)
        _set('field_CartNumber', e.CartNumber)
        _set('field_PartNumber', e.PartNumber)
        _set('field_R1', e.R1)
        _set('field_R2', e.R2)
        _set('field_R3', e.R3)
        _set('field_R4', e.R4)
        _set('field_RComp', e.RComp)
        _set('field_Sensitivity', e.Sensitivity)
        _set('field_TP_1_1', e.TP1_1)
        _set('field_TP_1_2', e.TP1_2)
        _set('field_TP_2_1', e.TP2_1)
        _set('field_TP_2_2', e.TP2_2)
        _set('field_TR_1', e.TR_1)
        _set('field_TR_2', e.TR_2)
        _set('field_TS_1', e.TS_1)
        _set('field_TS_2', e.TS_2)
        _set("field_PuassonStatic", e.PuassonStatic)
        _set("field_YungStatic", e.YungStatic)
        _set("field_E1", e.E1)
        _set("field_E2", e.E2)
        _set("field_E3", e.E3)
        _set("field_E4", e.E4)
        _set("field_Rotate", e.Rotate)
        _set("field_RockType", e.RockType)
...
def factory(
        table_class: typing.Type[database.Base],
        entity: database.Base = None,
        on_save: Callable[[database.Base], None] = None,
