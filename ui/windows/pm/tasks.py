...
from .task_editor import PmTaskEditor


class Tasks(wx.Panel, listmix.ColumnSorterMixin):
    def __init__(self, parent):
        super().__init__(parent)

        main_sizer = wx.BoxSizer(wx.VERTICAL)
        self._toolbar = wx.ToolBar(self, style=wx.TB_FLAT | wx.TB_HORZ_TEXT | wx.TB_BOTTOM)
        tool = self._toolbar.AddTool(wx.ID_ADD, "Добавить задачу", get_icon("add-row"))
        self._toolbar.Bind(wx.EVT_TOOL, self._on_create, id=wx.ID_ADD)
        tool = self._toolbar.AddTool(wx.ID_EDIT, "Редактировать задачу", get_icon("edit"))
        tool.Enable(False)
        self._toolbar.Bind(wx.EVT_TOOL, self._on_item_activated, id=wx.ID_EDIT)
        tool = self._toolbar.AddTool(wx.ID_DELETE, "Удалить задачу", get_icon("delete"))
        tool.Enable(False)
    ...
    def load(self):
        ...
        for index, o in enumerate(select(o for o in PmPerformedTask)):
            self._tasks[o.RID] = o
    ...
    def _on_delete(self, event):
        ...
        o = select(o for o in PmPerformedTask if o.RID == _id).first()
        if o != None:
            if delete_object(o):
                self.load()
    ...
    def _on_whitespace_right_click(self, event):
        ...
        if index == -1:
            menu = wx.Menu()
            item = menu.Append(wx.ID_ADD, "Добавить задачу")
    ...
    def _on_item_activated(self, event):
        ...
        dlg = PmTaskEditor(self, self._tasks[_id], _type="UPDATE")
        if dlg.ShowModal() == wx.ID_OK:
            self.load()
