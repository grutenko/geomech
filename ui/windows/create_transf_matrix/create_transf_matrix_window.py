...
class CreateTransfMatrixWindow(wx.Frame):
    def __init__(self, parent):
        ...
        self.result_matrix = wx.TextCtrl(
            self, style=wx.TE_MULTILINE | wx.TE_READONLY, size=(200, 50)
        )
        ...
        vbox.Add(
            wx.StaticText(self, label="Точки в системе 1 (формат: x1 y1; x2 y2; ...)")
        )
        vbox.Add(self.input_points, flag=wx.EXPAND | wx.BOTTOM, border=10)
        vbox.Add(
            wx.StaticText(self, label="Точки в системе 2 (формат: x1 y1; x2 y2; ...)")
        )
