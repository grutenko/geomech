...
from .actions import ID_TEST_SERIES, ID_ADD_ORIG_SAMPLE_SET, ID_ADD_PM_SAMPLE_SET, ID_ADD_PM_SAMPLE
from ui.icon import get_icon
from pony.orm import select, db_session
from database import PMTestSeries

PmTestSeriesSelect, EVT_PM_TEST_SERIES_SELECT = wx.lib.newevent.NewEvent()
PmTestSeriesManage, EVT_PM_TEST_SERIES_MANAGE = wx.lib.newevent.NewEvent()
...
class MainToolBar(wx.ToolBar):
    ...
    def __init__(self, parent):
        ...
        self.AddTool(ID_ADD_ORIG_SAMPLE_SET, "Добавить набор образов", get_icon("file-add"))
        self.EnableTool(ID_ADD_PM_SAMPLE_SET, False)
        self.EnableTool(ID_ADD_PM_SAMPLE, False)
        self.EnableTool(ID_ADD_ORIG_SAMPLE_SET, False)
        self.AddSeparator()
    ...
    def on_dropdown(self, event):
        m = wx.Menu()
        i = m.Append(wx.ID_PROPERTIES, "Управление")
        i.SetBitmap(get_icon("show-property"))
        m.AppendSeparator()
        self.pm_test_series = []
        for index, o in enumerate(select(o for o in PMTestSeries)):
            i = m.Append(index + 1, "%s (Проб: %d)" % (o.Name, len(o.pm_sample_sets)))
    ...
    def on_pm_test_series_menu(self, event):
        if event.Id == wx.ID_PROPERTIES:
            wx.PostEvent(self, PmTestSeriesManage())
        else:
